...
const promoteToAdmin = async ({ conversationId, targetUserId }) => {
    if (!conversationId) throw new Error("Missing conversationId");
    const res = await Api.put(`/conversations/${conversationId}/admins`, { targetUserId });
    return res.data.data;
};
...
const demoteAdmin = async ({ conversationId, adminId }) => {
    ...
    const res = await Api.delete(`/conversations/${conversationId}/admins/${adminId}`);
    return res.data.data;
};
...
const removeParticipant = async ({ conversationId, participantId }) => {
    ...
    const res = await Api.delete(`/conversations/${conversationId}/participants/${participantId}`);
    return res.data.data;
};

const updateGroupAvatar = async ({ conversationId, formData }) => {
    if (!conversationId) throw new Error("Missing conversationId");
    const res = await Api.patch(`/conversations/${conversationId}/avatar`, formData, {
        headers: {
...
const addParticipants = async ({ conversationId, participantIds }) => {
    if (!conversationId) throw new Error("Missing conversationId");
    const res = await Api.post(`/conversations/${conversationId}/participants`, { participantIds });
    return res.data.data;
};
