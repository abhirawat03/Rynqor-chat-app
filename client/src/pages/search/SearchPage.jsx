...
const SearchPage = () => {
  ...
  const showEmpty = !debouncedSearch.trim();

  const showNoResults = !isLoading && debouncedSearch && users.length === 0;

  const showResults = users.length > 0;

  return (
    <div className="flex justify-center flex-1 px-4 py-4 pb-20 overflow-hidden bg-surface md:pb-4">
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide">
            ...
            {showEmpty && (
              <div className="flex flex-col items-center justify-center h-full px-6 text-center ">
                ...
                "
                >
                  <ImSearch size={32} className="text-muted" />
                </div>
            ...
            {/* LOADING */}
            {isLoading && (
              <div className="py-6 text-sm text-center text-muted">
                Searching...
              </div>
            )}
            ...
            {/* NO RESULTS */}
            {showNoResults && (
              <div className="py-12 text-sm text-center text-muted">
                No users found
              </div>
            )}
              ...
              <div className="space-y-2.5 p-3">
                {users.map((user) => (
                  <SearchCard
                    ...
                    avatar={user.avatar}
                    bio={user.bio}
                    isLoading={startingChatId === user._id}
