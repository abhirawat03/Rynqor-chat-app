...
const createGroupConversation = async (req, res) => {
    ...
    const systemMsg = await createSystemMessage(conversation._id, userId, systemMsgText);
...
const promoteToAdmin = async (req, res) => {
    ...
    const conversation = await promoteToAdminService(userId, conversationId, targetUserId);

    // Create system message
    const targetUser = await User.findById(targetUserId);
    const callerName = await getCallerName(userId);
    const systemMsgText = `${callerName} promoted ${targetUser?.fullName || "a member"} to Admin`;
    const systemMsg = await createSystemMessage(conversationId, userId, systemMsgText);
...
const removeParticipant = async (req, res) => {
    ...
    const conversation = await removeParticipantService(userId, conversationId, participantId);
...
const updateGroupAvatar = async (req, res) => {
    ...
    const conversation = await updateGroupAvatarService(userId, conversationId, req.file?.path);
...
const updateGroupName = async (req, res) => {
    ...
    const conversation = await updateGroupNameService(userId, conversationId, name);
...
const addParticipants = async (req, res) => {
    ...
    const conversation = await addParticipantsService(userId, conversationId, participantIds);

    // Create system message
    const addedUsers = await User.find({ _id: { $in: participantIds } }).select("fullName");
...
const demoteAdmin = async (req, res) => {
    ...
    const conversation = await demoteAdminService(userId, conversationId, adminId);
    ...
    return res
        ...
        .json(
            new ApiResponse(
                200,
                result,
                result.deleted ? "Group deleted on last participant leaving" : "Left group successfully"
            )
        );
};
