...
const getVerificationTemplate = (otp, username) => {
  return `
    <!DOCTYPE html>
    <html>
      <head>
        <meta charset="utf-8">
        <title>Verify your Rynqor account</title>
        <style>
          body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f3f4f6; margin: 0; padding: 0; }
          .container { max-width: 500px; margin: 40px auto; background: #ffffff; padding: 32px; border-radius: 24px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06); border: 1px solid #e5e7eb; }
          .header { text-align: center; margin-bottom: 24px; }
      ...
      <body>
        <div class="container">
          ...
          <div class="greeting">Hi ${username},</div>
          <div class="instruction">
            Thank you for creating an account on Rynqor. To activate your account, please enter the verification code below:
          </div>
          <div class="otp-container">
            <div class="otp-code">${otp}</div>
            <div class="expiry">Expires in 24 hours</div>
          </div>
          <div class="instruction">
            If you did not create an account on Rynqor, please ignore this email or contact support if you have concerns.
          </div>
          <div class="footer">
            This is an automated security message from Rynqor Chat App.<br>
            Please do not reply directly to this email.
...
export { sendEmail, getOtpTemplate, getVerificationTemplate };
