...
const removeParticipantService = async (userId, conversationId, targetUserId) => {
    const conversation = await Conversation.findOne({
        _id: conversationId,
        participants: userId,
    });
    ...
    if (conversation.type !== "group") {
        throw new ApiError(400, "Only group conversations support removing participants");
    }
    ...
    if (conversation.admins.length > 0 && conversation.admins[0].toString() === targetUserId.toString()) {
...
const updateGroupAvatarService = async (userId, conversationId, avatarLocalPath) => {
    const conversation = await Conversation.findOne({
        _id: conversationId,
        participants: userId,
    });
...
const addParticipantsService = async (userId, conversationId, participantIds) => {
    ...
    if (conversation.type !== "group") {
        throw new ApiError(400, "Only group conversations support adding participants");
    }
    ...
    const newParticipantIds = participantIds.filter(
        (pId) => !conversation.participants.some((memberId) => memberId.toString() === pId.toString())
    );
...
const leaveGroupService = async (userId, conversationId) => {
    ...
    const isCreator = conversation.admins.length > 0 && conversation.admins[0].toString() === userId.toString();
    ...
    if (conversation.participants.length === 0) {
        if (conversation.avatar?.publicId) {
            await cloudinary.uploader.destroy(conversation.avatar.publicId).catch(err => {
                console.error("Cloudinary group avatar destroy failed during leave:", err);
            });
        }
                ...
                for (const med of msg.media) {
                    if (med.publicId) {
                        await cloudinary.uploader.destroy(med.publicId).catch(err => {
                            console.error("Cloudinary media destroy failed during leave:", err);
                        });
                    }
                }
            }
...
const deleteGroupService = async (userId, conversationId) => {
    ...
    if (!isCreator) {
        throw new ApiError(403, "Only the group creator (main admin) can delete the group");
    }

    if (conversation.avatar?.publicId) {
        await cloudinary.uploader.destroy(conversation.avatar.publicId).catch(err => {
            console.error("Cloudinary group avatar destroy failed during delete:", err);
        });
    }
    ...
    for (const msg of messages) {
        if (msg.media && msg.media.length > 0) {
            for (const med of msg.media) {
                if (med.publicId) {
                    await cloudinary.uploader.destroy(med.publicId).catch(err => {
                        console.error("Cloudinary media destroy failed during delete:", err);
                    });
                }
            }
        }
